...
def _get_win_cuda_defines(repository_ctx):
    ...
    msvc_ml_path = "windows/msvc_wrapper_for_ml.bat" # find_msvc_tool(repository_ctx, vc_path, "ml64.exe").replace(
        # "\\",
        # "/",
    # )
    ...
    msvc_lib_path = "C:/Program Files/LLVM/bin/llvm-lib.exe" # find_msvc_tool(repository_ctx, vc_path, "lib.exe").replace(
        # "\\",
        # "/",
    # )
    ...
    if not enable_cuda(repository_ctx):
        ...
        if not has_cuda_version or not has_cudnn_version:
            auto_configure_fail("%s and %s must also be set if %s is specified" %
                                (TF_CUDA_VERSION, _TF_CUDNN_VERSION, _TF_CUDA_CONFIG_REPO))
