...
NVCC_TEMP_DIR = tempfile.gettempdir() + '\\nvcc_inter_files_tmp_dir' # "%{nvcc_tmp_dir}"
...
def InvokeNvcc(argv, log=False):
  ...
  NVCC_PATH = glob('C:/Program Files/NVIDIA GPU Computing Toolkit/CUDA/v*/bin/nvcc.exe'
    )[0].replace(os.sep, '/')
...
def ExpandParamsFileForArgv():
  new_argv = []
  fo_opt = '/Fobazel-out/x64_windows-opt/bin/'
  cpu_compiler_flags = [CPU_COMPILER,
    "/showIncludes",
    "/nologo",
    "/c",
    "/bigobj",
  ...
  for arg in sys.argv:
    if arg.startswith("@"):
      try:
        ...
        # 'bazel-out/x64_windows-opt/bin/tensorflow/compiler/mlir/quantization
        # /tensorflow/calibrator/_objs/calibration_statistics_collector_average_min_max/
        # calibration_statistics_collector_average_min_max.obj.params'
        if arg.endswith("calibration_statistics_collector_average_min_max.obj.params"):
          fo = fo_opt + "tensorflow/compiler/mlir/quantization/tensorflow/calibrator/_objs/" \
            + "calibration_statistics_collector_average_min_max/calibration_statistics_collector_average_min_max.obj"
          compiler_flags = cpu_compiler_flags.append(fo).append(
            "tensorflow/compiler/mlir/quantization/tensorflow/calibrator/calibration_statistics_collector_average_min_max.cc"
          )
          
          return subprocess.call(compiler_flags)
    else:
      new_argv.append(arg)
...
def main():
  ...
  if len(output) == 1:
    ...
    # commandfile.close()
    cpu_compiler_flags.insert(0, "-Wno-microsoft-include")
    cpu_compiler_flags.insert(0, "-Wno-implicit-function-declaration")
    cpu_compiler_flags.insert(0, CPU_COMPILER)
    return subprocess.call(cpu_compiler_flags) # [CPU_COMPILER, "@" + commandfile_path])
  else:
