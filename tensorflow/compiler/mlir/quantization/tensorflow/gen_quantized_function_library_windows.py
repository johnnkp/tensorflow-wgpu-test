...
PYTHON_BINARY = 'python_x86_64-pc-windows-msvc/python.exe'
...
def FindModuleSpace(main_rel_path):
  ...
  if IsWindows():
    return os.path.join(main_rel_path,
      'gen_quantized_function_library.exe.runfiles')
...
def Main():
  ...
  main_rel_path = 'org_tensorflow/tensorflow/compiler/mlir/quantization/tensorflow/gen_quantized_function_library.py'
  if IsWindows():
    from glob import glob
    main_rel_path = glob('C:/msys64/home/*/_bazel_*/*/execroot/org_tensorflow/bazel-out/' +
      'x64_windows-opt-exec-*/bin/tensorflow/compiler/mlir/quantization/tensorflow')[0]
  ...
  main_filename = os.path.join(main_rel_path, 'gen_quantized_function_library.exe') # module_space, main_rel_path)
  ...
  if IsRunningFromZip():
    ...
    if os.environ.get('RUN_UNDER_RUNFILES') == '1':
      workspace = os.path.join(module_space, 'org_tensorflow')

  try:
    sys.stdout.flush()
    # NOTE: ExecuteFile may call execve() and lines after this will never run.

    print(python_program, main_filename, args, new_env, module_space,
      cov_tool, workspace,
      delete_module_space)
